...
def main():
    while True:
        try:
            ...
            # Иначе первый цикл даст NameError на prev_idle.
            _cpu_total = m.get("_cpu_total", 0)
            _cpu_idle = m.get("_cpu_idle", 0)
            prev = _net_prev.get("cpu")
            if prev and _cpu_total:
                dt = _cpu_total - prev[0]
                didle = _cpu_idle - prev[1]
                if dt > 0:
                    m["cpu_pct"] = round(100.0 * (dt - didle) / dt, 1)
            _net_prev["cpu"] = (_cpu_total, _cpu_idle)
